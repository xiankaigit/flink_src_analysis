...
public class TaskManagerServices {
    ...
    public static TaskManagerServices fromConfiguration(
        ...
        // 负责在本地提供一个shuffle环境，使用memory segment作为数据存储,内存原理分析的时候会进行说明
        //这个时候因为是启动tm,task还没有注册，所以这里不涉及具体的InputGate和ResultPartition，
        // 但是这个地方会创建networkBufferPool(所有task共享，本地缓冲池还没创建)并以此为依据进一步创建singleInputGateFactory和resultPartitionFactory
        //后免费分析task生产端和消费段再debug进去，
        ...
        // 一旦某个job manager获得leader角色，或者失去leader状态，会通知JobLeaderListener
        ...
        // 读取本地状态保存根路径
        // 通过参数taskmanager.state.local.root-dirs进行配置
